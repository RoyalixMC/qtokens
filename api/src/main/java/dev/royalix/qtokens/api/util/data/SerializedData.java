...
public class SerializedData {
  ...
  public <T> T applyAs(Class<T> clazz, Supplier<T> def) {
    T ob = GsonHelper.fromElement(jsonElement, clazz);
    return ob == null ? def.get() : ob;
  }

  public <T> T applyAs(Class<T> clazz) {
    return GsonHelper.fromElement(jsonElement, clazz);
  }

  public <T> T applyAs() {
    return (T) GsonHelper.fromElement(jsonElement);
  }
        ...
        StreamSupport.stream(array.spliterator(), false)
            ...
            .map(
                sd -> {
                  ...
                  return new InteliPair<>(
                      new SerializedData(key).applyAs(keyClazz),
                      new SerializedData(value).applyAs(valueClazz));
                })
            .collect(Collectors.toMap(InteliPair::getKey, InteliPair::getValue)));
    return map;
  }

  public Stream<InteliPair<SerializedData, SerializedData>> applyAsMap() {
    Preconditions.checkArgument(
    ...
    return StreamSupport.stream(jsonElement.getAsJsonArray().spliterator(), false)
        .map(
            element -> {
              ...
              JsonElement key = jsonObject.get("key");
              JsonElement value = jsonObject.get("value");
              return new InteliPair<>(new SerializedData(key), new SerializedData(value));
            });
  }
  ...
  public Stream<InteliPair<SerializedData, SerializedData>> applyAsMap(String field) {
    JsonArray array = getElement(field).map(JsonElement::getAsJsonArray).orElse(new JsonArray());
  ...
  public <T> T applyAs(String field, Class<T> clazz, Supplier<T> def) {
    return getElement(field)
        .map(element -> GsonHelper.fromElement(element, clazz))
  ...
  public void write(String field, Object object) {
    ...
    consumeJsonObject(
        jo -> {
          JsonElement wrap = GsonHelper.wrap(object);
          jo.add(field, wrap);
        });
  }
